...
def _get_causal_history(user_id, history_dict, window_days=14):
    ...
    for date_str in all_dates[:window_days]:
        try:
            ...
            for l in act_logs:
                if l["log_type"] == "food":
                    macros["calories"] += l.get("value", 0)
                    try:
                        raw = json.loads(l["raw_input"]) if isinstance(l["raw_input"], str) else l["raw_input"]
                        if raw.get("parsed"):
                            p = raw["parsed"]
                            macros["protein"] += p.get("protein", 0)
                            macros["carbs"] += p.get("carbs", 0)
                            macros["fat"] += p.get("fat", 0)
                        if raw.get("query"):
                            meals.append(raw["query"])
                    except: pass
            ...
            record = {
                ...
                "previous_day_behavior": {
                    ...
                    "intensity_min": act_sync.get("intensity_minutes"),
                    "sleep_hours": sleep_h,
                    "macros": macros,
                    "meals": meals
                }
...
def get_coaching_recommendation(user_id, goal: str = "overall_wellness", force_mode: str = "auto"):
    ...
    biomarkers = {
        "hrv": latest.get("hrv_avg"),
        "rhr": latest.get("resting_hr"),
        "battery": latest.get("body_battery"),
    }
    ...
    obs = [
        ...
        latest.get("hrv_avg", 45) if latest else 45,
        35.0, 25.0, 55.0, 75.0, 50.0,
        latest.get("body_battery", 50) if latest else 50,
        (latest.get("resting_hr", 70) - prev.get("resting_hr", 70)) if (latest and prev) else 0,
        (latest.get("hrv_avg", 45) - prev.get("hrv_avg", 45)) if (latest and prev) else 0,
        0, 0, 0, 0, 0, 0
    ]
...
def _get_expected_deltas(user_id, rec, latest_sync, history_list):
    ...
    try:
        ...
        persona = PersonaConfig(name=f"user_{user_id}", response_model=rm)
        
        # Map rec string to Action
        ...
        # Handle human-readable exercise strings from heuristic recs
        ...
        action = Action(
            sleep=sleep_map.get(rec["sleep"], SleepDuration.OPTIMAL_LOW),
            exercise=ExerciseType(exercise_val),
            nutrition=NutritionType(rec.get("nutrition", "balanced"))
        )
        
        current_bio = Biomarkers(
            resting_hr=latest_sync.get("resting_hr", 70) if latest_sync else 70,
            hrv=latest_sync.get("hrv_avg", 45) if latest_sync else 45,
            vo2_max=latest_sync.get("vo2_max", 35) if latest_sync else 35,
            body_fat_pct=latest_sync.get("body_fat_pct", 25) if latest_sync else 25,
            lean_mass_kg=latest_sync.get("lean_mass_kg", 55) if latest_sync else 55,
            sleep_efficiency=latest_sync.get("sleep_efficiency", 80) if latest_sync else 80,
            cortisol_proxy=latest_sync.get("cortisol_proxy", 40) if latest_sync else 40,
            energy_level=latest_sync.get("energy_level", 50) if latest_sync else 50
        )
        
        sim_history = []
        for s in (history_list or {}).get("syncs", [])[:7]: 
            sim_history.append({
                "actual_action": {"exercise": s.get("exercise", "none"), "sleep": "7-8 hours"}
            })
            
        deltas = compute_biomarker_changes(action, current_bio, persona, sim_history, random.Random(42))
